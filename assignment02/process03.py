...
class Basket:
    ...
    def use_eggs(self, index):
        print(f'{ctime()} Kitchen {index} : Chef {index} has locked with remaining eggs: {self.eggs.value}')
        self.eggs.value -= 1
        print(f'{ctime()} Kitchen {index} : Chef {index} has released lock with remaining eggs: {self.eggs.value}')

def cooking(index, basket):
    cooking_time = time()
    print(f'{ctime()} Kitchen {index} : Begin cooking with PID {os.getpid()}')
    sleep(2)
...
if __name__ == "__main__":
    ...
    print(f'{ctime()} Main : Eggs remaining in basket: {basket.eggs.value}')
